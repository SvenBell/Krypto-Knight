...
#Heading UPN is needed
#$VerbosePreference =
$Filename = "C:\Users\StephenBell_mw03ceg\TShopBiz & Entag Group\Projects - Customer Projects\F K Gardner\PR2535-TCO\Project Templates\PhoneSystemAssignments.csv"
...
Write-host ""
Write-Host "Bulk enable Enterprise Voice for users "
...
ForEach ($user in $users) {
    ...
    $i = $i + 1
    $error.clear()
    write-host "$i. Enabling Enterprise Voice for:" $upn "" -foregroundcolor Yellow -NoNewline
    Try {
    ...
    if (!$error) {
        ...
        write-host "`r$i. Enabled Enterprise Voice for:" $upn " " -foregroundcolor Green
    }
    else {
