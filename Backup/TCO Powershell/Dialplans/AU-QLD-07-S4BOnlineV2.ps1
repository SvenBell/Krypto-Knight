...
This script generated for Queensland, Australia.
...
Run the program from Skype for Business Online Powershell by typing .\AU-QLD-07-S4BOnline.ps1. 
#>
...
If ($DialPlanSelect -eq 1) {
	...
	$DPParent = "AU-QLD-07"
	New-CsTenantDialPlan $DPParent -Description "Normalization rules for Queensland, Australia"
}
...
If ($DialPlanSelect -ne 2) {
	...
	$NR = @()
	$NR += New-CsVoiceNormalizationRule -Name "AU-QLD-07-Local" -Parent $DPParent -Pattern '^([2-9]\d{7})$' -Translation '+617$1' -InMemory -Description "Local number normalization for Queensland, Australia"
...
$AU_LocalList = ,"AU-QLD-07-Local"
$AU_MobileList = ,"AU-QLD-07-Mobile"
$AU_PremiumList = ,"AU-QLD-07-Premium"
$AU_NationalList = ,"AU-QLD-07-National"
$AU_InternationalList = ,"AU-QLD-07-International"

If ((Get-CsOnlineVoiceRoutingPolicy | Where-Object {$_.Identity -notlike 'AU-QLD-07-*'}).Count -gt 0) {
	...
	If($FailoverRouting -eq 0) {
		...
		If (($LCRSites -ne '') -and ($LCRSites -ne $NULL)) {
			...
			$AU_LocalList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like "AU-*-Local" -and $_ -ne "AU-QLD-07-Local" -and $LCRList -cmatch ($_.SubString(0,$_.Length - ($_.Length-$_.LastIndexOf('-'))))}
			...
			$AU_MobileList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like 'AU-*-Mobile' -and $_ -ne "AU-QLD-07-Mobile" -and $LCRList -cmatch ($_.SubString(0,$_.Length - ($_.Length-$_.LastIndexOf('-'))))}
			...
			$AU_PremiumList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like 'AU-*-Premium' -and $_ -ne "AU-QLD-07-Premium" -and $LCRList -cmatch ($_.SubString(0,$_.Length - ($_.Length-$_.LastIndexOf('-'))))}
			$AU_NationalList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -Like 'AU-*-National' -and $_ -ne "AU-QLD-07-National" -and $LCRList -cmatch ($_.SubString(0,$_.Length - ($_.Length-$_.LastIndexOf('-'))))}
			...
			$AU_InternationalList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like 'AU-*-International' -and $_ -ne "AU-QLD-07-International" -and $LCRList -cmatch ($_.SubString(0,$_.Length - ($_.Length-$_.LastIndexOf('-'))))}
		...
		Else {
			# Create a list of all local, national and international PSTN usages for least-cost routing
			$AU_LocalList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like "AU-*-Local" -and $_ -ne "AU-QLD-07-Local"}
			...
			$AU_MobileList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like 'AU-*-Mobile' -and $_ -ne "AU-QLD-07-Mobile"}
			...
			$AU_PremiumList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like 'AU-*-Premium' -and $_ -ne "AU-QLD-07-Premium"}
			$AU_NationalList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -like 'AU-*-National' -and $_ -ne "AU-QLD-07-National"}
			...
			$AU_InternationalList += (Get-CsOnlinePstnUsage).Usage | Where-Object {$_ -Like 'AU-*-International' -and $_ -ne "AU-QLD-07-International"}
...
New-CsOnlineVoiceRoutingPolicy "AU-QLD-07-Local" -Description "Allows local calls from Queensland, Australia" -WarningAction:SilentlyContinue | Out-Null

# Only create National/International policies if user did not select LocalOnly command line option
If (!$LocalOnly) {
	New-CsOnlineVoiceRoutingPolicy "AU-QLD-07-National" -Description "Allows local/national calls from Queensland, Australia" -WarningAction:SilentlyContinue | Out-Null
	New-CsOnlineVoiceRoutingPolicy "AU-QLD-07-International" -Description "Allows local/national/international calls from Queensland, Australia" -WarningAction:SilentlyContinue | Out-Null
}

Write-Host "Creating PSTN usages"
Set-CsOnlinePSTNUsage -Identity global -Usage @{Add="AU-QLD-07-Local"} -WarningAction:SilentlyContinue | Out-Null
Set-CsOnlinePSTNUsage -Identity global -Usage @{Add="AU-QLD-07-Service"} -WarningAction:SilentlyContinue | Out-Null

# Only create following PSTN usages if user did not select LocalOnly command line option
If (!$LocalOnly) {
	Set-CsOnlinePSTNUsage -Identity global -Usage @{Add="AU-QLD-07-National"} -WarningAction:SilentlyContinue | Out-Null
	Set-CsOnlinePSTNUsage -Identity global -Usage @{Add="AU-QLD-07-Mobile"} -WarningAction:SilentlyContinue | Out-Null
	Set-CsOnlinePSTNUsage -Identity global -Usage @{Add="AU-QLD-07-Premium"} -WarningAction:SilentlyContinue | Out-Null
	Set-CsOnlinePSTNUsage -Identity global -Usage @{Add="AU-QLD-07-International"} -WarningAction:SilentlyContinue | Out-Null
}

Write-Host "Assigning usages to voice policies"
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-Local" -OnlinePstnUsages @{Add=$AU_LocalList} | Out-Null
If ($Int_LocalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-Local" -OnlinePstnUsages @{Add=$Int_LocalList} | Out-Null}
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-Local" -OnlinePstnUsages @{Add="AU-QLD-07-Service"} | Out-Null
If ($AU_LocalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages @{Replace=$AU_LocalList} | Out-Null}
Else {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages $NULL -WarningAction:SilentlyContinue | Out-Null}
If ($AU_MobileList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages @{Add=$AU_MobileList} | Out-Null}
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages @{Add=$AU_NationalList} | Out-Null
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages @{Add="AU-QLD-07-Service"} | Out-Null
If ($Int_LocalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages @{Add=$Int_LocalList} | Out-Null}
If ($Int_MobileList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages @{Add=$Int_MobileList} | Out-Null}
If ($Int_NationalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-National" -OnlinePstnUsages @{Add=$Int_NationalList} | Out-Null}
If ($AU_LocalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Replace=$AU_LocalList} | Out-Null}
Else {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages $NULL -WarningAction:SilentlyContinue | Out-Null}
If ($AU_MobileList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$AU_MobileList} | Out-Null}
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$AU_NationalList} | Out-Null
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$AU_PremiumList} | Out-Null
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add="AU-QLD-07-Service"} | Out-Null
If ($Int_LocalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$Int_LocalList} | Out-Null}
If ($Int_MobileList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$Int_MobileList} | Out-Null}
If ($Int_NationalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$Int_NationalList} | Out-Null}
Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$AU_InternationalList} | Out-Null
If ($Int_InternationalList -ne $NULL) {Set-CsOnlineVoiceRoutingPolicy -Identity "AU-QLD-07-International" -OnlinePstnUsages @{Add=$Int_InternationalList} | Out-Null}

# Configure Global voice policy as per user selection above
Switch ($VPSelect) {
	0 {Set-CsOnlineVoiceRoutingPolicy -Identity Global -OnlinePstnUsages @{Replace=(Get-CsOnlineVoiceRoutingPolicy AU-QLD-07-Local).OnlinePstnUsages} -Description "Allows local calls from Queensland, Australia"}
	1 {Set-CsOnlineVoiceRoutingPolicy -Identity Global -OnlinePstnUsages @{Replace=(Get-CsOnlineVoiceRoutingPolicy AU-QLD-07-National).OnlinePstnUsages} -Description "Allows local/national calls from Queensland, Australia"}
	2 {Set-CsOnlineVoiceRoutingPolicy -Identity Global -OnlinePstnUsages @{Replace=(Get-CsOnlineVoiceRoutingPolicy AU-QLD-07-International).OnlinePstnUsages} -Description "Allows local/national/international calls from Queensland, Australia"}
}


Write-Host "Creating voice routes"
New-CsOnlineVoiceRoute -Name "AU-QLD-07-Local" -Priority 0 -OnlinePstnUsages "AU-QLD-07-Local" -OnlinePstnGatewayList $PSTNGWList.Identity -NumberPattern '^\+610?7([2-9]\d{7})' -Description "Local routing for Queensland, Australia" | Out-Null
New-CsOnlineVoiceRoute -Name "AU-QLD-07-Mobile" -Priority 2 -OnlinePstnUsages "AU-QLD-07-Mobile" -OnlinePstnGatewayList $PSTNGWList.Identity -NumberPattern '^\+61([45]\d{8})$' -Description "Mobile routing for Queensland, Australia" | Out-Null
New-CsOnlineVoiceRoute -Name "AU-QLD-07-TollFree" -Priority 3 -OnlinePstnUsages "AU-QLD-07-Local" -OnlinePstnGatewayList $PSTNGWList.Identity -NumberPattern '^\+611[38]\d{4,8}$' -Description "TollFree routing for Queensland, Australia" | Out-Null
New-CsOnlineVoiceRoute -Name "AU-QLD-07-Premium" -Priority 4 -OnlinePstnUsages "AU-QLD-07-Premium" -OnlinePstnGatewayList $PSTNGWList.Identity -NumberPattern '^\+6119\d{4,8}$' -Description "Premium routing for Queensland, Australia" | Out-Null
New-CsOnlineVoiceRoute -Name "AU-QLD-07-National" -Priority 5 -OnlinePstnUsages "AU-QLD-07-National" -OnlinePstnGatewayList $PSTNGWList.Identity -NumberPattern '^\+610?[23578]\d{8}' -Description "National routing for Queensland, Australia" | Out-Null
New-CsOnlineVoiceRoute -Name "AU-QLD-07-International" -Priority 7 -OnlinePstnUsages "AU-QLD-07-International" -OnlinePstnGatewayList $PSTNGWList.Identity -NumberPattern '^\+((1[2-9]\d\d[2-9]\d{6})|((?!(61))([2-9]\d{6,14})))' -Description "International routing for Queensland, Australia" | Out-Null
New-CsOnlineVoiceRoute -Name "AU-QLD-07-Service" -Priority 6 -OnlinePstnUsages "AU-QLD-07-Service" -OnlinePstnGatewayList $PSTNGWList.Identity -NumberPattern '^\+?(000|1[0125]\d{1,8})$' -Description "Service routing for Queensland, Australia" | Out-Null
Write-Host "Finished!"
