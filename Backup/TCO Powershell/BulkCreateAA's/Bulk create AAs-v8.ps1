...
# v7 added additional .csv configurable attributes. Note if AADisplayName is blank only a resource account if created - handy for AAs that need multiple resource accounts assigned
# v8 Added Faculty licenses and extra questions to check license type correct
##############################
...
$confirmation4 = Read-Host "Have you set the correct Business Hours in script? i.e. 08:00 - 17:00 [y/n]"
while($confirmation4 -ne "y")
{
    if ($confirmation4 -eq 'n') {break}
    $confirmation2 = Read-Host "Have you set the correct Business Hours in script? i.e. 08:00 - 17:00 [y/n]"
}
