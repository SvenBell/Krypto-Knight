...
#Import data from CSV file into $users variable as a table
    ...
    foreach ($user in $users)
    {
        ...
        Get-CsOnlineUser -identity $upn | Select DisplayName,UserPrincipalName,LineURI,EnterpriseVoiceEnabled,Hostedvoicemail | export-csv "C:\temp\nhvr-Pre.csv" -Append -NoTypeInformation
        ...
        Get-CsOnlineUser -identity $upn | Select DisplayName,UserPrincipalName,LineURI,EnterpriseVoiceEnabled,Hostedvoicemail | export-csv "C:\temp\nhvr-Post.csv" -Append -NoTypeInformation
        Get-CsOnlineUser -identity $upn | Select DisplayName,UserPrincipalName,LineURI,EnterpriseVoiceEnabled,Hostedvoicemail
        Write-Host "-------"
    } 
