...
########################################
$domain= "wearecocomau.onmicrosoft.com"
$Filename = "C:\GitHub\PowerShell\TCO Powershell\AB\Bulk things\BulkCallVoicemail-CQ-Weareco.csv"
$language = "en-AU"
$voiceprompt = "Thank you for calling Weareco.  Unfortunately, we are unable to take your call at the moment.  Please hold to leave a message with your name, phone number and a short message, and we'll return your call.  Thank you."
########################################
#For testing
...
Set-CsCallQueue -id $CQ1id -AllowOptOut $true -AgentAlertTime $CQtimeout -UseDefaultMusicOnHold $true -OverflowThreshold 50 -TimeoutThreshold $CQtimeout -ConferenceMode $true -LanguageId $language -OverflowAction SharedVoicemail -OverflowSharedVoicemailTextToSpeechPrompt $voiceprompt -OverflowActionTarget $vmailid -EnableOverflowSharedVoicemailTranscription $true -TimeoutAction SharedVoicemail -EnableTimeoutSharedVoicemailTranscription $true -TimeoutSharedVoicemailTextToSpeechPrompt $voiceprompt -TimeoutActionTarget $vmailid
