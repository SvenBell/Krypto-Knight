...
# V7 11/10/2023 Tuned for Operator Connect. Check user checks if user licensed, Teams licensed and account enabled
#
...
Write-host ""
$Filename = "C:\temp\SRRC-MSTeamsPhoneStdUsers.csv"
...
Write-host ""
Write-Host "Bulk check users exist and licensed status "
...
Write-host "Total UPNs to check: " -NoNewline -ForegroundColor Yellow
...
ForEach ($user in $users) {
    ...
    Try {
        $userdetails = Get-AzureADUser -ObjectID $upn | Select DisplayName, UserPrincipalName, UsageLocation, AssignedLicenses, ObjectID, AccountEnabled
    }
    Catch {}
    if (!$error) {
        ...
        If ($AssignedLicenses.Count -eq 0) { 
            $USerTotallyUnlicensed = $USerTotallyUnlicensed + 1
            write-host "$i. Checked-user:" $upn $number $userdetails.UsageLocation" " -NoNewline
            write-host $AssignedLicenses.Count "licenses Totally_unlicensed" -NoNewline -foregroundcolor Magenta
            If ($AccountEnabled) {
                write-host " Account-Enabled" -foregroundcolor Green
            }
            Else {
                $UserAccountDisabled = ($UserAccountDisabled + 1)
                write-host " Account-Disabled" -foregroundcolor Yellow
            }
        }
        else {
            $TeamsLicenseStatus = Get-AzureADUserLicenseDetail -ObjectID $userdetails.ObjectID | Select-Object -ExpandProperty ServicePlans | Where-Object ServicePlanName -eq "TEAMS1"
            If ($TeamsLicenseStatus.ProvisioningStatus -eq "Success") {
                write-host "$i. Checked-user:" $upn $number $userdetails.UsageLocation" " -NoNewline
                write-host $AssignedLicenses.Count "licenses Teams=licensed" -NoNewline -foregroundcolor Green 
                If ($AccountEnabled) {
                    write-host " Account-Enabled" -foregroundcolor Green
                }
                else {
                ...
                $UserNotLicensedCount = $UserNotLicensedCount + 1
                write-host "$i. Checked-user:" $upn $number $userdetails.UsageLocation" " -NoNewline
                write-host $AssignedLicenses.Count "licenses Teams=unlicensed" -NoNewline -foregroundcolor Magenta
                If ($AccountEnabled) {
                    write-host " Account-Enabled" -foregroundcolor Green
                }
                else {
        ...
        $UPNerrorcount = $UPNerrorcount + 1
        write-host "$i. Checked-user:" $upn $number $userdetails.UsageLocation" " -NoNewline
        write-host "0 licenses Error-UPN_not_found Account-Missing" -foregroundcolor Red
    }
...
write-host "Users Checked: "$linesInFile -ForegroundColor Green
write-host "Users Checked OK: "($linesInFile - $errorcount)"of"$linesInFile -ForegroundColor Green
write-host "Number of UPNs disabled: " $UserAccountDisabled -ForegroundColor Yellow
write-host "Number of UPNs Totally unlicensed: " $USerTotallyUnlicensed -ForegroundColor Magenta
write-host "Number of UPNs without a Teams license: " $UserNotLicensedCount -ForegroundColor Magenta
write-host "Number of UPNs not found: " $UPNerrorcount -ForegroundColor Red
write-host "= Total Number of Errors: " ($UPNerrorcount + $UserNotLicensedCount + $USerTotallyUnlicensed) -ForegroundColor Red
Write-Host ""
#Get-MsolAccountSku
...
Write-Progress -Activity "Script in Progress" -Status "$i% Complete:" -PercentComplete ($i / $linesInFile * 100)
Write-host ""
Write-Host "Bulk Assign License(s) to users "
write-host "============================" -foregroundcolor Yellow
write-host "Start Time: " -NoNewline -foregroundcolor Yellow
write-host "$StartDate" -ForegroundColor Cyan
write-host "Importing file: " -NoNewline -foregroundcolor Yellow
...
#>
     write-host "    Assigning Teams Phone Standard license" -foregroundcolor Yellow -NoNewline
     Try {
...
ForEach ($user in $users) {
    ...
    Try {
        ...
        Set-CsPhoneNumberAssignment -Identity $upn -PhoneNumber $number -LocationId $loc.LocationId -PhoneNumberType OperatorConnect #-verbose CallingPlan / OperatorConnect
    }
    Catch {}
    if (!$error) {
        #Start-Sleep -Milliseconds 2000
        ...
        $errorcount = $errorcount + 1
        write-host "`r$i. # Assigning Error" $upn $number " LocationID: " $loc " " -foregroundcolor Red
