...
$greetingText = "Welcome to Maleny Library. Please hold the line while we connect you with one of the team."
...
if(!$VmailcallableEntityGroup)
{
    ...
    {
        Write-Host Voicemail Stage waiting Entity Group
        Start-Sleep -s 30
...
if(!$VmailcallableEntity)
{
    ...
    {
        Write-Host Voicemail Stage waiting Entity
        Start-Sleep -s 30
        $VmailcallableEntity = New-CsAutoAttendantCallableEntity -Identity $VmailcallableEntityGroup -Type SharedVoicemail -EnableTranscription
...
if(!$CQ1appinstanceid)
{
    ...
    {
        Write-Host Call Queue 1 Stage waiting instance id
        Start-Sleep -s 30
        $CQ1appinstanceid = (Get-CsOnlineUser $CQ1Name@$domain -ErrorAction SilentlyContinue).ObjectId
    }
}
...
if(!$CQ1id)
{
    while(!$CQ1id)
    {
        Write-Host Call Queue 1 Stage waiting id
        Start-Sleep -s 30
        $CQ1id = (Get-CsCallQueue -NameFilter $CQ1Name -ErrorAction SilentlyContinue).Identity
    }
}
