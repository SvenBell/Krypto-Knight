############################################
#Script to Create basic call flow-SCCCaloundra-Depot #
...
$greetingText = "Welcome to Property Management. Please hold the line while we connect you with one of the team."
$afterHoursText = "Thank you for calling Property Management. Sorry we cannot take your call at the moment. Please leave your name, telephone number and a brief message after the tone and we will return your call as soon as possible.  For an emergency service call on weekdays after 5.00pm, weekends or public holidays, please call 5475 7272. Thank you"
$greetingTextAAQ = "Thank you for calling Property Management. Sorry we cannot take your call at the moment. Please leave your name, telephone number and a brief message after the tone and we will return your call as soon as possible.  For an emergency service call on weekdays after 5.00pm, weekends or public holidays, please call 5475 7272. Thank you"
$holidaygreetingtext = "Thank you for calling Property Management. Sorry we cannot take your call at the moment. Please leave your name, telephone number and a brief message after the tone and we will return your call as soon as possible.  For an emergency service call on weekdays after 5.00pm, weekends or public holidays, please call 5475 7272. Thank you"
...
$VmailDisplay = "VM-PropertyManagement"
$VmailName = "VM-PropertyManagement"
...
# Call Queue 1 Stage
$CQ1Name = "CQ-PropertyManagement"
#Create Call Queue
New-CsCallQueue -Name $CQ1Name -RoutingMethod Attendant -AllowOptOut $true -AgentAlertTime $AgentalertCQ1 -LanguageId $language -EnableTimeoutSharedVoicemailTranscription $true -UseDefaultMusicOnHold $true -ConferenceMode $true -TimeoutThreshold $AgentalertCQ1 -timeoutsharedvoicemailtexttospeechprompt $greetingTextAAQ -TimeoutAction SharedVoicemail -TimeoutActionTarget $VmailcallableEntityGroup
...
#Start-Sleep -s 300
$CQ1appinstanceid = (Get-CsOnlineUser $CQ1Name@$domain).ObjectId -ErrorAction SilentlyContinue;
if($? -ne 'False')
{
    while($? -ne 'False')
    {
...
$CQ1id = (Get-CsCallQueue -NameFilter $CQ1Name).Identity -ErrorAction SilentlyContinue;
if($? -ne 'False')
{
    while($? -ne 'False')
    {
...
if($? -ne 'False')
{
    while($? -ne 'False')
    {
        Write-Host Call Queue 1 Stage waiting
        Start-Sleep -s 30
        $aaid = (Get-CsAutoAttendant -NameFilter $aaName).Identity -ErrorAction SilentlyContinue;
    }
}
# Associate AutoAttendant and AA Resource account
