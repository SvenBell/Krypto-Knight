...
$SKUTCO = "cropsmart:MCOPSTNEAU2"
$SKUPhone = "cropsmart:MCOEV"
...
$Filename = "C:\Temp\Cropsmartuserlist_18032020.csv"
    ...
    {
        $upn= $user.UPN
        Get-Msoluser -UserPrincipalName $upn | select UserPrincipalName,UsageLocation,@{n="Licenses Type";e={$_.Licenses.AccountSKUid}},DisplayName | export-csv "C:\temp\Cropsmartpre-licenses.csv" -Append -NoTypeInformation
        Set-MsolUser -UserPrincipalName $upn -UsageLocation "AU"
        write-host $upn "location set" -foregroundcolor DarkMagenta
        Set-MsolUserLicense -UserPrincipalName $upn -AddLicenses $SKUPhone,$SKUTCO -Verbose
        write-host $upn "license assigned" -foregroundcolor Green
        #Get-Msoluser -UserPrincipalName $upn | select UserPrincipalName,UsageLocation,@{n="Licenses Type";e={$_.Licenses.AccountSKUid}},DisplayName | export-csv "C:\temp\Cropsmartpost-licenses.csv" -Append -NoTypeInformation
    } 
