...
$greetingcontent = Get-Content "C:\Users\AndrewBaird\TShopBiz & Entag Group\ENTAG Connect - Documents\Customers\ENTAG Group\TCO\Entag Office move\Brisbane\OneDrive_1_12-08-2021_Refined\Mobility-Welcome-PSBA.wav" -Encoding byte -ReadCount 0
$greetingaudioFile = Import-CsOnlineAudioFile -ApplicationId "OrgAutoAttendant" -FileName "Mobility-Welcome-PSBA.wav" -Content $greetingcontent
...
$VmailDisplay = "PSBA-Voicemail"
$VmailName = "PSBA-Voicemail"
...
$CQ1appinstanceid = (Get-CsOnlineUser -erroraction SilentlyContinue $CQ1Name@$domain).ObjectId
if($? -ne 'False')
{
    while($? -ne 'false')
    {
...
$holdmenu = New-CsAutoAttendantMenu -Name "AA-PSBAHoldmenu" -Prompts @($holdgreetingprompt) -EnableDialByName -MenuOptions @($holdmenuOption1,$holdmenuOption2)
